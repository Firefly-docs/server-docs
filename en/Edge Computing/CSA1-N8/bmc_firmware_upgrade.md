...
<Callout title="Confirm Before Upgrading" type="warn">
  A BMC firmware upgrade rewrites the underlying storage contents. Before upgrading, confirm that the firmware matches the device model and that the power supply is stable. During the upgrade, do not cut the power, unplug the Type-C data cable, or repeat the upgrade command.
</Callout>

## Prepare Tools [step]

- A good-quality Type-C data cable.
- The official Rockchip upgrade tool.
- A Windows or Linux host.
- Firmware that matches the CSA1-N8 BMC, for example `update.img`.
...
Connect the host to the server's OTG port using a Type-C data cable.

![Connect the OTG port](../../../servers_img/CSA1-N8/bmc_upgrade/bmc_upgrade_usb_port.png)
...
<CodeBlockTabs defaultValue="loader_by_cmd">
  ...
  After the Type-C data cable is connected, you can enter the system through a serial debug terminal, `adb shell`, or an HDMI keyboard, and run:
  ...
  <CodeBlockTab value="loader_by_button">
    ...
    ![Location of the RECOVER button](../../../servers_img/CSA1-N8/bmc_upgrade/loader_recovery_button.png)
  </CodeBlockTab>
</CodeBlockTabs>
...
<CodeBlockTabs defaultValue="windows">
  ...
  <CodeBlockTab value="windows">
    ...
    If there is no **Rockusb Device**, reinstall the Rockusb driver, or try again after changing the USB port and the Type-C data cable.
  </CodeBlockTab>
...
- The Type-C data cable is of poor quality.
...
1. Replace the Type-C data cable.
2. Change the USB port on the host.
3. Reinstall the Rockusb driver on Windows.
4. Confirm that the firmware package matches the CS-A1 BMC.
