...
<CodeBlockTabs defaultValue="OutOfBand">
    ...
    <CodeBlockTab value="OutOfBand">
      ...
      **接线步骤**
      ...
      2. 将万兆光口转 RJ45 模块插入服务器的 SFP+1～SFP+4 中任意一个光口（图中 2，以 SFP+1 为例），另一根网线一端接入该模块，另一端接入外部网络。
    ...
    <CodeBlockTab value="InBand">
      ...
      **工具准备**
      ...
      - 网线 × 2（一根用于互连 MGMT 口与 SFP+1 口，一根用于连接外部网络）

      **接线步骤**

      1. 将一个万兆光口转 RJ45 模块插入服务器的 SFP+1 光口，并使用一根网线将 MGMT 口与该模块直连（图中 1）。
      2. 将另一个模块插入 SFP+2、SFP+3、SFP+4 中任意一个光口（图中 2，以 SFP+3 为例），外网网线接入该模块。

      说明：MGMT 口直连 BMC，将其与 SFP+1 口互连后，BMC 管理网口即可经由内部交换机、通过外网光口上的模块与外部网络通信。MGMT 口为千兆电口，SFP+1 口的模块与之协商为 1 Gbps。该方式下，带外管理流量借道业务网络，与业务共用同一根外网线。
    </CodeBlockTab>
</CodeBlockTabs>