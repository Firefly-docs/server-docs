...
本文介绍 CSA1-N8 BMC 固件升级流程。升级方式主要分为两类：
...
- 瑞芯微官方升级工具。
- Windows 或 Linux 主机。
- 与 CSA1-N8 BMC 匹配的固件，例如 `update.img`。
...
使用 Type-C 数据线连接主机和服务器的 OTG 接口。

![连接 OTG 接口](../../../servers_img/CSA1-N8/bmc_upgrade/bmc_upgrade_usb_port.png)
...
<CodeBlockTabs defaultValue="loader_by_cmd">
  ...
  Type-C 数据线连接完成后，可通过串口调试终端、`adb shell` 或 HDMI 键盘进入系统，执行：
  ...
  <CodeBlockTab value="loader_by_button">
    ...
    ![RECOVER 按键位置](../../../servers_img/CSA1-N8/bmc_upgrade/loader_recovery_button.png)
  </CodeBlockTab>
</CodeBlockTabs>
...
<CodeBlockTabs defaultValue="windows">
  ...
  <CodeBlockTab value="windows">
    ...
    打开 `RKDevTool.exe`。如果 BMC 已进入 Loader 模式，工具底部会提示 **Found One LOADER Device** 或 **MaskRom**。
