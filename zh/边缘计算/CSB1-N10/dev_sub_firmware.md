...
如果你的流程包含“设备侧导出 rootfs + PC 端重新打包固件”，请同时参考 [rootfs 导出与打包](recovery_export.mdpp)。
...
<CodeBlockTabs defaultValue="Rockchip">
  ...
  ```
  Rockchip / RK3588 平台通常提供 `.7z`、`.zip` 或 `.img` 格式的基础固件包，后续流程会自动解包成可处理的镜像。
  </CodeBlockTab>
  ...
  ```
  Sophgo / BM1684 平台通常提供 `.zip` 格式的基础固件包，后续流程会按 flow 自动解包、拆分和处理分区。
  </CodeBlockTab>
</CodeBlockTabs>
...
<CodeBlockTabs defaultValue="Rockchip">
  ...
  如果是 SD 卡升级包，也可以按交付人员要求切换到对应 flow，例如 `flow/bm1684_sdcard_data.yaml`。
  </CodeBlockTab>
</CodeBlockTabs>
...
<CodeBlockTabs defaultValue="Rockchip">
  <CodeBlockTabsList>
    ...
    <CodeBlockTab value="Rockchip">
    Rockchip 平台第一次暂停通常用于修改 `rootfs` 分区大小：
    ...
    <CodeBlockTab value="Sophgo">
    Sophgo 平台第一次暂停通常用于修改 `ROOTFS` 分区大小：
...
<CodeBlockTabs defaultValue="Rockchip">
  ...
  如果需要替换目录或配置，也可以直接把本地文件复制到 rootfs 对应路径。
...
<CodeBlockTabs defaultValue="Rockchip">
  ...
  <CodeBlockTab value="Rockchip">
  Rockchip 流程完成后会自动卸载 rootfs，并重新打包生成新的固件：
  ...
  <CodeBlockTab value="Sophgo">
  Sophgo 流程完成后会自动卸载分区，并重新打包生成新的升级包：

  ```text
  out/bm1684/ubuntu20/update/tftp-YYYY-MM-DD.zip
  ```

  如果使用 SD 卡 flow，对应输出为：
