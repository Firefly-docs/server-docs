...
bmc 命令是aBMC软件自带的cli命令行工具，用户可以通过 bmc 命令行工具连接子板。
  ```bash
    ./bmc terminal <类型> [aBMC 连接参数] <--core 核心板名称>
    * <> 中的是必填参数
    * [] 中的是选填参数。
    * 退出方式：先按 Ctrl+A（不区分大小写），再按 Q（不区分大小写），最后按 Enter 确认退出。
    * 参数选项：
      ...
      --port <aBMC 服务监听的端口>
      --user <aBMC 服务登录用户>
      --password <aBMC 服务登录密码>
      以上参数省略时会使用默认值：https、127.0.0.1、443、admin、admin。
  ```
<CodeBlockTabs defaultValue="ADB">
    <CodeBlockTabsList>
        <CodeBlockTabsTrigger value="ADB">SSH</CodeBlockTabsTrigger>
        <CodeBlockTabsTrigger value="Serial">网络ADB</CodeBlockTabsTrigger>
    ...
    <CodeBlockTab value="SSH">
      ...
      <Callout title="提示" type="info">
          选择 ssh 方式的话可以不使用 bmc 命令行工具，直接使用 ssh 子板用户名@子板ip 来连接更加便捷。
      </Callout>
      通过 ssh 连接子板
      ```bash
...
<CodeBlockTabs defaultValue="SSH">
    <CodeBlockTabsList>
        <CodeBlockTabsTrigger value="SSH">SSH</CodeBlockTabsTrigger>
        <CodeBlockTabsTrigger value="NetADB">网络ADB</CodeBlockTabsTrigger>
    </CodeBlockTabsList>
