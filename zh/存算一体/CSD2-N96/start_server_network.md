...
<CodeBlockTabs defaultValue="OutOfBand">
    ...
    <CodeBlockTab value="OutOfBand">
      ...
      ![两根网线接线示意](../../../servers_img/CSD2-N96/start_server_network/dual_external_network_wiring.png)
      ...
      **接线步骤**
      ...
      1. 将一根网线接入服务器的 MGMT 口（图中序号为 1），另一端接入外部网络。
      2. 将两个万兆光模块分别插入 SFP+4、SFP+5 光口，并使用光纤跳线互连两个光口（图中两口的序号均为 2），将两台内部交换机打通。
      3. 将万兆光口转 RJ45 模块插入服务器的 SFP+6 光口（图中序号为 3），另一根网线一端接入该模块，另一端接入外部网络。

      该方式下，带外管理（MGMT 口）与业务网络（SFP+ 口）各自独立上联，互不影响。

      **可选做法：三根外网线（两台内部交换机分别独立上联）**

      ![三根网线接线示意](../../../servers_img/CSD2-N96/start_server_network/triple_external_network_wiring.png)

      1. 将一根网线接入服务器的 MGMT 口（图中序号为 1），另一端接入外部网络。
      2. 将一个万兆光口转 RJ45 模块插入服务器的 SFP+4 光口（图中序号为 2），一根网线一端接入该模块，另一端接入外部网络。
      3. 将另一个万兆光口转 RJ45 模块插入服务器的 SFP+5 光口（图中序号为 3），最后一根网线一端接入该模块，另一端接入外部网络。
    ...
    <CodeBlockTab value="InBand">
      ...
      ![单独一根外网网线接线示意](../../../servers_img/CSD2-N96/start_server_network/single_external_network_wiring.png)
      ...
      **接线步骤**
      ...
      1. 将一个万兆光口转 RJ45 模块插入服务器的 SFP+2 光口，并使用一根网线互连 MGMT 口与 SFP+2 口（图中两口的序号均为 1）。
      2. 将两个万兆光模块分别插入 SFP+4、SFP+5 光口，并使用光纤跳线互连两个光口（图中两口的序号均为 2），将两台内部交换机打通。
      3. 将另一个万兆光口转 RJ45 模块插入服务器的 SFP+6 光口（图中序号为 3），外网网线接入该模块。

      说明：MGMT 口直连 BMC，将其与 SFP+2 口互连后，BMC 管理网口即接入 SFP+1～SFP+4 所在的内部交换机；SFP+4 与 SFP+5 互连打通两台内部交换机后，BMC 与业务流量即可经由 SFP+6 口上的模块与外部网络通信。MGMT 口为千兆电口，SFP+2 口的模块与之协商为 1 Gbps。该方式下，带外管理流量借道业务网络，与业务共用同一根外网线。
