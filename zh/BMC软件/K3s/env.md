...
<Callout>以上配置主要用于确定 Server 的基础资源。实际部署时，还需要结合 Pod 数量、业务负载、监控、存储等组件的资源占用进行调整。</Callout>
...
`STATUS: pass` 表示内核满足 K3s 的运行要求。如果为 `fail`，需要根据输出中标记为 `missing` 的项来调整内核的配置，重新编译后再烧录；`Optional Features` 下缺失的项属于可选能力（例如加密网络相关内容）
