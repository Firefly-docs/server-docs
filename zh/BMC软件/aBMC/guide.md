...
### 1.无法打开登录页面
检查管理网口接线、操作电脑 IP 配置、交换机 VLAN 和管理 IP 是否正确。先确认操作电脑能够访问目标管理 IP。

### 2.浏览器显示证书告警
确认地址栏中的 IP 是目标 BMC 管理 IP；确认网络可信后，按页面提示进入高级选项并继续访问。

### 3.提示账号或密码错误
确认使用的是 Web 管理账号 `admin`，并注意密码可能已经被首次登录后的修改操作更新。

### 4.页面加载不完整
刷新页面并确认浏览器版本、管理网络和 aBMC 服务状态正常；问题持续时联系设备管理员。

### 5.忘记账号密码
将服务器连接至显示屏，执行命令 `rm /var/firefly-aBMC/resources/database/firefly_bmc.db` 删除数据库文件，然后重启服务器。
